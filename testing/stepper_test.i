#line 1 "stepper_test.cpp"
#line 1 "c:\\users\\ask-lp\\documents\\platformio\\projects\\190820-071059-uno\\src\\winder-code\\stepper.h"
...
#line 14 "C:\\Program Files (x86)\\Microsoft Visual Studio\\2017\\Community\\VC\\Tools\\MSVC\\14.16.27023\\include\\stdint.h"
...
#line 5 "c:\\users\\ask-lp\\documents\\platformio\\projects\\190820-071059-uno\\src\\winder-code\\stepper.h"
...
#line 32 "c:\\users\\ask-lp\\documents\\platformio\\projects\\190820-071059-uno\\src\\winder-code\\stepper.h"
#line 2 "stepper_test.cpp"
...
#line 13 "C:\\Program Files (x86)\\Windows Kits\\10\\include\\10.0.18362.0\\ucrt\\string.h"
...
#line 5 "stepper_test.cpp"

#line 1 "c:\\users\\ask-lp\\documents\\platformio\\projects\\190820-071059-uno\\src\\winder-code\\winder_pins.h"
...
#line 111 "c:\\users\\ask-lp\\documents\\platformio\\projects\\190820-071059-uno\\src\\winder-code\\winder_pins.h"
...
#line 336 "c:\\users\\ask-lp\\documents\\platformio\\projects\\190820-071059-uno\\src\\winder-code\\winder_pins.h"

#line 338 "c:\\users\\ask-lp\\documents\\platformio\\projects\\190820-071059-uno\\src\\winder-code\\winder_pins.h"
#line 7 "stepper_test.cpp"
...
void stepper_incBuffTailIndex()
{
	...
	stepper_execBuffTailIndex = stepper_execBuffTailIndex % 1 ;
}
...
void stepper_init()
{
	...
	for(int i=0;i<1;++i)
	{
		(&stepperBuff2[i])->flags = ((&stepperBuff2[i])->flags & (0xff & ~(1 << 2))) | ((1 & 1) << 2) ;
	}
}
...
uint16_t stepper_addLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1)
{
	...
	seg->flags = 0 ;
	seg->dx =  abs_i(x1-x0) ; seg->flags = (seg->flags & (0xff & ~(1 << 0))) | (((x0<x1 ? 1 : 0) & 1) << 0);
	seg->dy = -abs_i(y1-y0) ; seg->flags = (seg->flags & (0xff & ~(1 << 1))) | (((y0<y1 ? 1 : 0) & 1) << 1); 
	...
	if(seg->dy != 0 && seg->dx != 0)
	{
		...
		seg->rcount = 1 ;
	seg->err = seg->dx+seg->dy ; 
	seg->xCounter = seg->yCounter = 0 ; 
	seg->flags = (seg->flags & (0xff & ~(1 << 2))) | ((0 & 1) << 2) ;
	seg->flags = (seg->flags & (0xff & ~(1 << 3))) | ((1 & 1) << 3) ;
	seg->flags = (seg->flags & (0xff & ~(1 << 4))) | ((0 & 1) << 4) ;
...
void stepper_run()
{
	if(isMotorEnabled)
	{
		...
		if(!((execSeg->flags & (1 << 4)) >> 4))
		{
			if(!((execSeg->flags & (1 << 3)) >> 3))
			{
			...
			if(((execSeg->flags & (1 << 0)) >> 0))
				PIN_X_DIR_PORT |= BITVAL_PIN_X_DIR_BITN ;
			else
				PIN_X_DIR_PORT &= BITVAL_PIN_X_DIR_BITN ;
			if(((execSeg->flags & (1 << 1)) >> 1))
			{
				PIN_Y_DIR_PORT |= BITVAL_PIN_Y_DIR_BITN ;
				yStep_dir = 1 ;
			}
			else
			{
			...
			execSeg->flags = (execSeg->flags & (0xff & ~(1 << 4))) | ((1 & 1) << 4) ;
			tCounter = xSteps = ySteps = 0 ;
			tMax = execSeg->tMax ;
			execSeg->xCounter = execSeg->yCounter = 0 ; 
		...
		if (execSeg->xCounter == execSeg->dx && execSeg->yCounter == -execSeg->dy) 
		{
			...
			if(execSeg->rcount > 0)
			{
				...
				execSeg->flags = (execSeg->flags & (0xff & ~(1 << 2))) | ((1 & 1) << 2) ;
				execSeg->flags = (execSeg->flags & (0xff & ~(1 << 4))) | ((0 & 1) << 4) ;
				execSeg->flags = (execSeg->flags & (0xff & ~(1 << 3))) | ((0 & 1) << 3) ;
				stepper_execBuffHeadIndex++ ;
				stepper_execBuffHeadIndex = stepper_execBuffHeadIndex % 1 ;
				...
				if(((execSeg->flags & (1 << 3)) >> 3))
				{
					...
					stepper_currentSegNum = execSeg->segNum ;
					execSeg->flags = (execSeg->flags & (0xff & ~(1 << 4))) | ((1 & 1) << 4) ;
					execSeg->flags = (execSeg->flags & (0xff & ~(1 << 2))) | ((0 & 1) << 2) ;
					tCounter = xSteps = ySteps = 0 ;
					tMax = execSeg->tMax ;
				} else {
		...
		if(isMotorEnabled)
		{
			...
			{ 
				...
				PIN_X_STEP_PORT |= BITVAL_PIN_X_STEP_BITN ;
				execSeg->xCounter++ ;  
				xSteps++ ;
			} 
			if (e2 <= execSeg->dx) 
			{ 
				...
				PIN_Y_STEP_PORT |= BITVAL_PIN_Y_STEP_BITN ;
				execSeg->yCounter++ ;  
				ySteps += yStep_dir ;
			} 
			tCounter++ ;
